...
- **我的笔记**：个人剪裁、整理的所有私密笔记。
- **知识库同步**：支持同步你创建或参与的所有 Get笔记 知识库专题。
- **订阅博主**：追踪你在Get笔记中关注的创作者（抖音、微信、得到等），将他们的深度内容一键抓取至本地。

### 🎨 全量/增量同步

- **全量同步**：强制覆盖本地内容，确保与云端数据完全一致，适合需要同步历史修改的场景。
- **增量同步**：基于笔记 ID 智能识别，仅同步本地缺失的内容，大幅提升同步效率。
- **日期筛选**：支持通过快捷选择（最近3天/1周/1月）或自定义日期，精准控制同步范围。
